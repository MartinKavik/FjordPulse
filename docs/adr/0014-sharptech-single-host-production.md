...
- store encrypted checksum-backed logical exports in independent
  S3-compatible storage with Restic, and prove restore into an isolated target;
- back up Coolify's control plane separately to independent object storage;
...
Host recovery cannot depend on an undocumented API, provider firewall or
best-effort provider backup. A provider outage or disk loss is recovered from
the independent logical backup and recorded application release, while Coolify
is recovered from its own independent control-plane backup.
