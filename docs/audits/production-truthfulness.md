...
| Data provenance | Fake mode rendered an `Entur OK` presentation. | Public health exposes data mode, fake mode displays a prominent `Demo data` badge, and real mode carries neutral `Transport data: Entur` attribution separate from health. |
...
| Vehicle context/delay | Direction always ended in `NE`; negative delay rendered as `+-N min`. | The rider summary now derives Previous stop from authoritative journey progress and shows `Not available` when absent; bearing remains map data, while the tested delay formatter owns early/on-time/delayed copy. |
...
| Broad/typo search | Alphabetical database capping could discard Førde before relevance ranking, later-word prefixes required a catalog scan, and five literal Geocoder matches suppressed every fuzzy station. | Normalized name, locality, and municipality prefix lanes use indexed ranges with relevance applied before the bound. SurrealDB-derived token-prefix and length/first/last-character indexes cover later words and supply one-edit candidates for the database's exact Damerau residual without a catalog scan; a valid fuzzy station slot survives unrelated literal prefix and Geocoder results. |
...
| Mobile status | CSS injected `station clusters loaded` regardless of runtime state. | Injected operational content is removed; validated telemetry drives one contextual update notice only when delivery needs attention, including while a detail sheet is open. |
...
| Operational/dead-run vehicles | A newer operator-internal movement could inherit Line, delay, and route wording from Vehicle Positions even though Journey Planner had no public service journey. | A backend-authored passenger-service enum keeps the physical marker/watch live while explicitly suppressing passenger line, delay, and stop claims for dead runs/internal garage movements. |
...
| Public update health | Pessimistic startup placeholders could survive a failed map parse, health dependency states were discarded, idle demand-driven realtime looked broken, a successful local map implied Entur success, generic last-update age was ambiguous, and header/footer indicators duplicated one state with different colours. | A strict health schema still feeds one reducer, but healthy idle delivery is silent, selected resources own age and exceptional warnings, and exactly one rider-facing notice explains reconnecting, periodic refresh, or saved-data fallback. Component diagnostics remain in Admin, source credit is not health, and newer resource timestamps cannot be erased by polling. |
...
| Source attribution | Health failure defaulted the public badge to real/Entur, and demo map loads could write `Entur OK`. | Provenance begins as unknown, renders no Entur claim until health confirms it, real mode uses neutral `Transport data: Entur`, and fake mode consistently reports `not_used` plus a prominent `Demo data` badge. |
