    ...
    <div class="database-disclosure-content">
      ...
      <section class="database-definition-section" aria-labelledby={`fields-${props.table.name}`}>
        ...
        <Show when={props.table.fields.length > 0} fallback={<p class="database-empty">{tx(i18n, "Ingen definerte felt.", "No defined fields.")}</p>}>
          <dl class="database-definition-list"><For each={props.table.fields}>{(field) => <div><dt><code>{field.name}</code><span>{field.type}</span><Show when={field.readonly}><span class="database-code-badge">{tx(i18n, "SKRIVEBESKYTTET", "READ ONLY")}</span></Show></dt><dd><Show when={field.assertion !== null}><span><strong>ASSERT</strong><code>{field.assertion}</code></span></Show><Show when={field.defaultValue !== null}><span><strong>DEFAULT</strong><code>{field.defaultValue}</code></span></Show><Show when={field.computedValue !== null}><span><strong>COMPUTED</strong><code>{field.computedValue}</code></span></Show><Show when={field.valueExpression !== null}><span><strong>VALUE</strong><code>{field.valueExpression}</code></span></Show><Show when={field.referenceOnDelete !== null}><span><strong>REFERENCE ON DELETE</strong><code>{field.referenceOnDelete}</code></span></Show><span class="database-field-permissions">{fieldPermission(tx(i18n, "Les", "Select"), field.permissions.select)}{fieldPermission(tx(i18n, "Opprett", "Create"), field.permissions.create)}{fieldPermission(tx(i18n, "Oppdater", "Update"), field.permissions.update)}</span></dd></div>}</For></dl>
        </Show>
      </section>
