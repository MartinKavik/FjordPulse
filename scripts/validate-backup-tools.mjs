...
assert.match(backup, /\[\[ "\$BACKUP_KIND" == pre_release_\* \]\]/);
assert.match(backup, /--tag pre_release/);
assert.match(backup, /--keep-daily "\$KEEP_DAILY"/);
assert.match(backup, /--keep-weekly "\$KEEP_WEEKLY"/);
assert.match(backup, /--keep-last "\$KEEP_RELEASES"/);
assert.match(backup, /--group-by host/);
assert.match(backup, /if \[\[ "\$BACKUP_KIND" == 'scheduled' \]\]/, "scheduled and pre-release retention must stay separate");
...
assert.match(smoke, /BACKUP_RETENTION_RELEASES=3/);
assert.match(smoke, /--tag pre_release/);
assert.match(smoke, /Encrypted local backup, short retention, and independent-endpoint restore smoke passed/);
