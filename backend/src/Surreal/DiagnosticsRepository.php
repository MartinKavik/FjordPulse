...
RETURN count(SELECT VALUE id FROM station_timetable);
RETURN count(SELECT VALUE id FROM current_vehicle);
RETURN count(SELECT VALUE id FROM journey_snapshot);
...
SURQL, ['migration_limit' => $migrationLimit]);
        ...
        $latestImport = DatabaseRecord::one($results[9] ?? null);
        $migrations = array_map(
            AppliedMigration::fromRecord(...),
            DatabaseRecord::many($results[10] ?? []),
        );
        ...
        return new PersistenceDiagnostics(
            ...
            DatabaseRecord::int($results[2] ?? null, 'diagnostics.station_timetables'),
            DatabaseRecord::int($results[3] ?? null, 'diagnostics.current_vehicles'),
            DatabaseRecord::int($results[4] ?? null, 'diagnostics.journey_snapshots'),
            DatabaseRecord::int($results[5] ?? null, 'diagnostics.vehicle_observations'),
            DatabaseRecord::int($results[6] ?? null, 'diagnostics.watches'),
            DatabaseRecord::int($results[7] ?? null, 'diagnostics.realtime_events'),
            DatabaseRecord::int($results[8] ?? null, 'diagnostics.entur_request_logs'),
